...
int main() {
  ...
  // Reference velocity.
  double last_planned_vel = 0.0; // mph

  h.onMessage([&last_planned_vel, &car_lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy]
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            /* Trajectory Generation: Generate Waypoints based on 20ms time. */
            vector<double> ptsx;
            vector<double> ptsy;
            ...
            vector<double> next_wp0 = getXY(car_s + safeDistance, 2 + 4*car_lane, map_waypoints_s, map_waypoints_x, map_waypoints_y);
            vector<double> next_wp1 = getXY(car_s + safeDistance*2, 2 + 4*car_lane, map_waypoints_s, map_waypoints_x, map_waypoints_y);
            ...
            double target_x = safeDistance;
            double target_y = s(target_x);
            double target_dist = sqrt(target_x*target_x + target_y*target_y);

            double x_add_on = 0;

            for( int i = 1; i < 60 - prev_path_size; i++ ) {
